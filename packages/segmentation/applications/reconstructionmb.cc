...
int main(int argc, char **argv)
{
  ...
  //reconstruction.MaskStacks(stacks,stack_transformations);
   if (debug)
   {
     for(i=0;i<stacks.size();i++)
     {
      sprintf(buffer,"maskedstack%i.nii.gz",i);
      stacks[i].Write(buffer);
     }
   }
  ...
  for (int iter=0;iter<iterations;iter++)
  {
	...
	{
	  reconstruction.MaskStacks(stacks,stack_transformations);
	  reconstruction.PackageToVolumeMasking(stacks,multiband_packages,iter);
	}
	else
          reconstruction.SliceToVolumeRegistration();
      }
