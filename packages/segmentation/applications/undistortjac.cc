...
int main(int argc, char **argv)
{
  ...
  irtkRealImage jacobian = resfieldmap;
  jacobian=0;
    ...
    for (k = 0; k < image.GetZ(); k++) {
	  ...
	  if(swap)
	    if (minus)
	    {
	      ...
	      if((j+1)<image.GetY())
	        jacobian(i,j,k)=1+(resfieldmap(i,j,k)-resfieldmap(i,j+1,k))/attr._dy;
	    }
	    else
	    {
	      y+=resfieldmap(i,j,k)/attr._dy;
	    ...
	    if (minus)
	    {
	      ...
	      if((i+1)<image.GetX())
	        jacobian(i,j,k)=1+(resfieldmap(i,j,k)-resfieldmap(i+1,j,k))/attr._dx;
	    }
	    else
	    {
	      x+=resfieldmap(i,j,k)/attr._dx;
  ...
  jacobian.Write("jacobian.nii.gz");
  //The end of main()
