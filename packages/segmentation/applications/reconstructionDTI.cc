...
void usage()
{
  cerr << "Usage: reconstruction [reconstructed] [DTI] [bvalues] [directions] <options>\n" << endl;
  cerr << endl;
...
int main(int argc, char **argv)
{
  ...
  for (i=1;i<=nStacks;i++)
  {
    ...
    stacks[i-1].Write(buffer);
    
  }
  
  //Read the name of the texfile with b-values
  char *btextfile = argv[1];
  argc--;
  argv++;
  cout<<"B-value textfile is "<<btextfile<<endl;
  ...
  int noEntries = nStacks+1;
  cout<<"Number of directions is "<<nStacks<<endl;
  cout<<"Number of entries in texfiles is "<<noEntries<<endl;
  
  double num;
  vector<vector<double> > directions(3,vector<double>(noEntries,0));
  vector<double> bvalues(noEntries,0);
...
*/
  ...
  reconstruction.CoeffInit();
  reconstruction.GaussianReconstruction4D2(nStacks);
  ...
  //SH reconstruction
  reconstruction.InitSH(dirs_xyz,2);
  ...
  {
    reconstruction.SuperresolutionDTI(iteration);
    reconstruction.SimulateSlicesDTI();
