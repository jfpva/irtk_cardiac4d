...
void usage()
{
  ...
  cerr << "\t-iterations [iter]        Number of registration-reconstruction iterations. [Default: 7]"<<endl;
  cerr << "\t-mid_iter [iter]          Swap from separate fieldmap estimation to joint at this iteration. [Default: 4]"<<endl;
  ...
  cerr << "\t-no_distortion_correction Switch off distortion correction."<<endl;
  ...
  cerr << "\t-boundary_weight [weight] weight for boundary smoothness"<<endl;
  cerr << "\t-minus                    fieldmaps have opposit signs"<<endl;
...
int main(int argc, char **argv)
{
  ...
  while (argc > 1){
    ...
    //Swap from separate to joint fieldmap estimation at this iteration
    if ((ok == false) && (strcmp(argv[1], "-mid_iter") == 0)){
      argc--;
      argv++;
      mid_iter=atoi(argv[1]);
      ok = true;
      argc--;
      argv++;
    }
    ...
    //Switch off intensity matching
    if ((ok == false) && (strcmp(argv[1], "-no_distortion_correction") == 0)){
      argc--;
      argv++;
 ...
 */ 
  ...
  for (iter=0;iter<(iterations);iter++)
  {
	...
	reconstructed=reconstruction.GetReconstructed();
	sprintf(buffer,"target0-%i.nii.gz",iter);
	...
	{
	  if(fmask!=NULL)
            reconstruction.SmoothFieldmapGroup(*fmask,1,iter, false,boundary_weight,minus);
	  else
	...
	if(iter>mid_iter)
	{
	  ...
	  if(fmask!=NULL)
	    reconstruction.SmoothFieldmapGroup(*fmask,1-current_group,iter, true,boundary_weight,minus);
	  else
	    reconstruction.SmoothFieldmapGroup(m,1-current_group,iter, true,boundary_weight,minus);
	}
	
	///Change 4
	if(iter==mid_iter)
	  reconstruction.CombineFieldmaps();
...
/*
  ...
  if (debug)
  {
   ...
   reconstruction.BSplineReconstructionGroup(groups[0]);
   reconstructed=reconstruction.GetReconstructed();
   reconstructed.Write("BsplineImage-0.nii.gz");
   reconstruction.BSplineReconstructionGroup(groups[1]);
   reconstructed=reconstruction.GetReconstructed();
   reconstructed.Write("BsplineImage-1.nii.gz");
}
